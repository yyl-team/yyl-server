    ...
    if (proxy && env.proxy) {
      ...
      const formatLocalServer = (str) => {
        if (typeof str === 'string') {
          return str
            .replace(`127.0.0.1:5000`, `${extOs.LOCAL_IP}:${port || 5000}`)
            .replace(`127.0.0.1:50001`, `${extOs.LOCAL_IP}:${port || 5000}1`)
            .replace(`localhost:5000`, `${extOs.LOCAL_IP}:${port || 5000}`)
