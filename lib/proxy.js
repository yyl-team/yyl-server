...
const PROXY_CRET_PATH = util.path.join(USERPROFILE, '.anyproxy/certificates')
...
class YProxy {
  ...
  static async certClean() {
    const { log } = this
    const iLog = log || (() => undefined)
    if (fs.existsSync(PROXY_CRET_PATH)) {
      iLog('info', [
        `${LANG.PROXY.CLEAN_CERT_START}: ${chalk.yellow(PROXY_CRET_PATH)}`
      ])
      const files = await extFs.removeFiles(PROXY_CRET_PATH)

      iLog('success', [
        `${LANG.PROXY.CLEAN_CERT_FINISHED}, total ${chalk.yellow(
          files.length
        )} files`
      ])
              ...
              request(
                {
                  ...
                  method: req.requestOptions.method,
                  headers: req.requestOptions.headers,
                  encoding: isImage(proxyUrl) ? null : 'utf8'
                },
                (err, vRes) => {
                  if (err) {
                    log('warn', [
                      `${proxyUrl} - ${LANG.PROXY.REQUEST_ERROR}:`,
                      err
                    ])
                    next(null)
