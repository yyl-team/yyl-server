...
class YProxy {
  static async clean() {
    ...
    if (fs.existsSync(PROXY_CACHE_PATH)) {
      ...
      iLog('success', [`${LANG.PROXY.CLEAN_CACHE_FINISHED}, total ${chalk.yellow(files.length)} files`]);
    } else {
  ...
  async start() {
    ...
    if (!await extOs.checkPort(proxyOpts.webInterface.webPort)) {
      throw new Error(`${LANG.SERVER.PORT_OCCUPIED}: ${chalk.yellow(proxyOpts.webInterface.webPort)}`);
    }
    ...
    return await util.makeAwait((next) => {
      ...
      server.on('ready', () => {
        ...
        Object.keys(config.localRemote).forEach((key) => {
          log('success', [`${LANG.INFO.PROXY_MAP}: ${chalk.cyan(key)} => ${chalk.yellow.bold(config.localRemote[key])}`]);
        });
