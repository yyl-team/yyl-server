...
class YServer {
  ...
  // 初始化设置
  constructor({ log, config, env, cwd, option }) {
    if (log) {
      this.log = log
    }
  ...
  async start() {
    ...
    if (config.entry) {
      if (fs.existsSync(config.entry)) {
        try {
          ...
          log('error', [
            `${LANG.SERVER.START_ERROR}: ${chalk.yellow(config.entry)}`,
            er
          ])
          throw er
        }
        log('info', [
          `${LANG.SERVER.USE_PROJECT_SERVER}: ${chalk.yellow(config.entry)}`
        ])
        if (!app || typeof app.use !== 'function') {
          log('warn', [
            `${LANG.SERVER.NOT_EXPORT_APP}: ${chalk.yellow(config.entry)}`
          ])
      ...
      log('success', [
        `${LANG.INFO.SERVER_PATH}: ${chalk.yellow.bold(config.root)}`
      ])
      log('success', [
        `${LANG.INFO.SERVER_ADDRESS}: ${chalk.yellow.bold(
    ...
    if (config.livereload) {
      if (!(await extOs.checkPort(config.lrPort))) {
        throw new Error(
          `${LANG.SERVER.PORT_OCCUPIED}: ${chalk.yellow(config.lrPort)}`
        )
      }
      lrServer = tinylr()
      app.use(
